...
class Job implements JobInterface
{
    ...
    public function execute($queue)
    {
        ...
        $cronId = CronLogs::start(CronLogs::CRON_LICARD_API_PARSE_STATS, $source->id);
        $this->parser->cronId = $cronId;
        $data = $this->parser->parseStats($source->contract_id);
        if (!$data) {
            CronLogs::end($cronId, 0, 'Отчет пуст', 'Не удалось получить данные из источника');
            return;
        }
        if ($count = $this->handler->handle($data, $source->id, $source->type)) {
